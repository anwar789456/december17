...
export default function CustomSelect({ titleOptionSelect, onOptionSelectDirection }) {
  const [isOpen, setIsOpen] = useState(false);
  const [selected, setSelected] = useState("Droit");
  const dropdownRef = useRef();
  const hasMounted = useRef(false);
  useEffect(() => {
    if (hasMounted.current) {
      onOptionSelectDirection(selected);
    } else {
      hasMounted.current = true;
    }
  }, [selected, onOptionSelectDirection]);
